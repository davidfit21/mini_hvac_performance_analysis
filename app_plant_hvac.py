...
if 'extend_lines' not in st.session_state:
    st.session_state.extend_lines = False
if 'extend_by' not in st.session_state:
    st.session_state.extend_by = 5
...
def results_changed(old, new):
    ...
    if old.keys() != new.keys():
        return True
    for k in old:
        if 'bin_results' not in old[k] or 'bin_results' not in new[k]:
            return True
        for arr_key in ['X','y','y_pred']:
            if not np.array_equal(old[k]['bin_results'][arr_key], new[k]['bin_results'][arr_key]):
                return True
    return False
...
def process_fluid(file_paths, fluid_name, baseline_name, st_container=None):
    ...
    for f in file_paths:
        ...
        for df_dict in all_dfs:
            X = df_trend_filtered['oat'].values.reshape(-1, 1)
            y = df_trend_filtered['cop'].values
            lr = LinearRegression().fit(X, y)
            df_dict['bin_results'] = {
                'X': X.flatten(),
                'y': y,
    ...
    st_container.success(
        f"{fluid_name} processing complete! "
        f"{len(df_trend_filtered)} valid 1-minute COP points"
    )
...
if phase == "Phase 1: Analysis":
    ...
    if run_button:
        if not baseline_name or not fluid_name:
            st.error("Provide names for both baseline and product fluid.")
        else:
            baseline_results, baseline_df_filtered = process_fluid(
                st.session_state.get('baseline_paths', []),
                baseline_name,
                baseline_name
            )
            fluid_results, fluid_df_filtered = process_fluid(
                st.session_state.get('fluid_paths', []),
                fluid_name,
                baseline_name
            )

            if not baseline_results or not fluid_results:
                st.error("Not enough valid COP points in uploaded files.")
            else:
                combined_results = {
                    baseline_name: {
                        **baseline_results[0],
                        'df': baseline_df_filtered,
                        'bin_results': {
                            "X": np.array(baseline_results[0]['oat']),
                            "y": np.array(baseline_results[0]['cop']),
                            "y_pred": np.polyval([baseline_results[0]['slope'], baseline_results[0]['intercept']], np.array(baseline_results[0]['oat'])),
                            "slope": baseline_results[0]['slope'],
                            "r2": baseline_results[0]['r2'],
                        }
                    },
                    fluid_name: {
                        **fluid_results[0],
                        'df': fluid_df_filtered,
                        'bin_results': {
                            "X": np.array(fluid_results[0]['oat']),
                            "y": np.array(fluid_results[0]['cop']),
                            "y_pred": np.polyval([fluid_results[0]['slope'], fluid_results[0]['intercept']], np.array(fluid_results[0]['oat'])),
                            "slope": fluid_results[0]['slope'],
                            "r2": fluid_results[0]['r2']
                        }
                    }
                }
                ...
                density = {
                    baseline_name: oat_density_table(baseline_df_filtered),
                ...
                # Always create new plotter with fresh data
                st.session_state.plotter = PlantMultiFluidDynamicPlot(
                    combined_results, 
                    default_baseline=baseline_name
                )

    # Display plot if data is ready (even when checkbox is clicked)
    if st.session_state.plot_data_ready and st.session_state.results_dict:
        if st.session_state.plotter is None:
            st.session_state.plotter = PlantMultiFluidDynamicPlot(
                st.session_state.results_dict,
                default_baseline=st.session_state.get('baseline_name_input', '')
            )
        
        # This ensures plot shows even when checkbox is toggled
...
if phase == "Phase 2: Alignment":
    ...
    if 'results_dict' not in st.session_state or not st.session_state.results_dict:
        st.error("No data available. Run Phase 1 first.")
    else:
        available_fluids = list(st.session_state.results_dict.keys())

        baseline_name = st.selectbox("Select Baseline Fluid:", options=available_fluids, key="align_baseline_select")
        product_options = [f for f in available_fluids if f != baseline_name]
        if not product_options:
            st.error("Need at least two different fluids for alignment.")
        else:
            fluid_name = st.selectbox("Select Product Fluid to Align:", options=product_options, key="align_product_select")
            ...
            if run_button:
                ...
                if 'bin_results' not in baseline_data:
                    st.error(f"Baseline '{baseline_name}' has no regression results")
                else:
                    baseline_slope = baseline_data['bin_results']['slope']
                    st.info(f"Aligning {fluid_name} to match {baseline_name}")

                    aligned_name, aligned_result = align_to_match_baseline_plant(
                        fluid_name,
                        fluid_data,
                        baseline_slope,
                        baseline_name
                    ...
                    st.session_state.results_dict[fluid_name] = aligned_result  # Use original name, not aligned_name
                    st.success(f"Alignment complete! {fluid_name} has been updated.")

                    fluids_to_keep = [baseline_name, fluid_name]
                    st.session_state.results_dict = {
                    k: v for k, v in st.session_state.results_dict.items() 
                    if k in fluids_to_keep
                    }
                    ...
                    st.session_state.plotter.results_dict = st.session_state.results_dict
                    st.session_state.plotter.default_baseline = baseline_name
            
            # Always show plot if we have data
            if st.session_state.plotter is None and st.session_state.results_dict:
                st.session_state.plotter = PlantMultiFluidDynamicPlot(
                    st.session_state.results_dict,
                    default_baseline=baseline_name
                )
