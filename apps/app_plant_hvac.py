...
def run():
    ...
    def process_fluid(file_paths, fluid_name, baseline_name, st_container=None):
        ...
        for f in file_paths:
            ...
            all_dfs.append({
                "oat": df_trend_filtered["oat"].values,
                "cop": df_trend_filtered["cop"].values,
                "slope": lr_filtered.coef_[0],
                "intercept": lr_filtered.intercept_,
                "r2": lr_filtered.score(df_trend_filtered["oat"].values.reshape(-1,1), df_trend_filtered["cop"].values),
                "name": fluid_name,
                "bin_results": {
                    "X": df_trend_filtered["oat"].values,
                    "y": df_trend_filtered["cop"].values,
                    "y_pred": lr_filtered.predict(df_trend_filtered["oat"].values.reshape(-1,1)),
                    "slope": lr_filtered.coef_[0],
                    "r2": lr_filtered.score(df_trend_filtered["oat"].values.reshape(-1,1), df_trend_filtered["cop"].values)
                }
            })
            ...
            if not product_options:
                ...
                if run_button:
                    ...
                    aligned_name, aligned_result = align_to_match_baseline_plant(
                        fluid_name, fluid_data, baseline_slope, baseline_name
                    )
                    ...
                    st.session_state.results_dict = {k: v for k, v in st.session_state.results_dict.items() if k in fluids_to_keep}
